...
export const metadata: Metadata = {
  title: "Гибридные автомобили из Кореи — каталог | EncarKorea",
  description:
    "Гибридные автомобили из Южной Кореи с ценами в рублях. Hyundai Tucson Hybrid, KIA Sorento Hybrid, Genesis GV80 Hybrid и другие модели.",
};

export default async function HybridPage() {
  const allStoreCars = getCarsWithBooked();
  const hybrid = FUEL_FILTERS.hybrid;
  let cars = allStoreCars.filter((c) => c.fuelType === hybrid);
  let total = cars.length;

  if (total === 0) {
    const result = await fetchCars({ limit: 200, fuelType: hybrid });
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        >
          ...
          <span className="text-text-muted">/</span>
          <span className="text-text-primary">Гибриды</span>
        </nav>
        <h1 className="text-2xl sm:text-3xl font-bold text-text-primary mb-2">
          Гибридные автомобили из Кореи
        </h1>
        <p className="text-[15px] text-text-secondary mb-8 max-w-2xl">
          Гибридные авто сочетают экономичность и комфорт. Популярные модели: Hyundai Tucson Hybrid,
          KIA Sorento Hybrid, Genesis GV80 Hybrid.
        </p>
      </div>
