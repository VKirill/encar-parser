...
export const metadata: Metadata = {
  title: "Авто с механической коробкой из Кореи — каталог | EncarKorea",
  description:
    "Автомобили с механической коробкой передач (МКПП) с корейского рынка. Широкий выбор моделей с «механикой» по доступным ценам с доставкой в Россию.",
};

export default async function ManualPage() {
  const allStoreCars = getCarsWithBooked();
  let cars = allStoreCars.filter((c) => c.transmission === "수동");
  let total = cars.length;

  if (total === 0) {
    const result = await fetchCars({ limit: 200, transmission: "수동" });
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        >
          ...
          <span className="text-text-muted">/</span>
          <span className="text-text-primary">МКПП</span>
        </nav>
        <h1 className="text-2xl sm:text-3xl font-bold text-text-primary mb-2">
          Авто с МКПП из Кореи
        </h1>
        <p className="text-[15px] text-text-secondary mb-8 max-w-2xl">
          Автомобили с механической коробкой передач с корейского рынка. Широкий выбор «механики» по
          доступным ценам с доставкой в Россию.
        </p>
      </div>
