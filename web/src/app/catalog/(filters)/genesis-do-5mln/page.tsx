...
export const metadata: Metadata = {
  title: "Genesis из Кореи до 5 млн ₽ — каталог | EncarKorea",
  description:
    "Премиальные Genesis из Южной Кореи до 5 млн рублей. G80, G90, GV80, GV70 — полный модельный ряд с доставкой в Россию.",
};

export default async function GenesisDo5MlnPage() {
  const allStoreCars = getCarsWithBooked();
  let cars = allStoreCars.filter(
    (c) => c.manufacturerKey === "genesis" && c.priceRUB > 0 && c.priceRUB <= 5000000
  );
  ...
  if (total === 0) {
    ...
    cars = result.cars
      .filter((c) => c.priceRUB > 0 && c.priceRUB <= 5000000)
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        >
          ...
          <span className="text-text-muted">/</span>
          <span className="text-text-primary">До 5 млн ₽</span>
        </nav>
        <h1 className="text-2xl sm:text-3xl font-bold text-text-primary mb-2">
          Genesis из Кореи до 5 000 000 ₽
        </h1>
        <p className="text-[15px] text-text-secondary mb-8 max-w-2xl">
          Широкий выбор Genesis из Южной Кореи до 5 млн рублей. G80, G90, GV80, GV70 —
          полный модельный ряд премиальных автомобилей с доставкой в Россию.
        </p>
      </div>
