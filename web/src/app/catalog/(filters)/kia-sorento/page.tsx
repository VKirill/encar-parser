...
export const metadata: Metadata = {
  title: "Kia Sorento из Кореи — цены | EncarKorea",
  description:
    "Kia Sorento всех поколений из Южной Кореи. Вместительный кроссовер с бензиновым, дизельным и гибридным двигателем. Доставка в Россию.",
};

export default async function KiaSorentoPage() {
  const allStoreCars = getCarsWithBooked();
  let cars = allStoreCars.filter(
    (c) => c.manufacturerKey === "kia" && c.model.includes("쏘렌토")
  );
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        >
          ...
          <span className="text-text-muted">/</span>
          <span className="text-text-primary">Sorento</span>
        </nav>
        <h1 className="text-2xl sm:text-3xl font-bold text-text-primary mb-2">
          Kia Sorento из Кореи
        </h1>
        <p className="text-[15px] text-text-secondary mb-8 max-w-2xl">
          Sorento — вместительный кроссовер для всей семьи. Все поколения с бензиновым,
          дизельным и гибридным двигателем. Опциональный полный привод и третий ряд сидений.
          Доставка из Южной Кореи в Россию.
        </p>
      </div>
