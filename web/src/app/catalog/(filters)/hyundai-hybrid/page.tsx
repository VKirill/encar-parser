...
export const metadata: Metadata = {
  title: "Гибридные Hyundai из Кореи — каталог и цены | EncarKorea",
  description:
    "Гибридные Hyundai из Южной Кореи от 1,5 млн ₽. Tucson, Santa Fe, Sonata гибрид с доставкой в Россию.",
};

export default async function HyundaiHybridPage() {
  const allStoreCars = getCarsWithBooked();
  const hybrid = FUEL_FILTERS.hybrid;
  let cars = allStoreCars.filter((c) => c.manufacturerKey === "hyundai" && c.fuelType === hybrid);
  let total = cars.length;

  if (total === 0) {
    const result = await fetchCars({ limit: 200, manufacturer: "hyundai", fuelType: hybrid });
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-text-primary mb-2">
          Гибридные Hyundai из Кореи
        </h1>
        <p className="text-[15px] text-text-secondary mb-8 max-w-2xl">
          Hyundai — лидер по производству гибридных автомобилей. Tucson, Santa Fe, Sonata и Avante
          в гибридных версиях доступны с экономией до 40% от цен российского рынка.
        </p>
      </div>
